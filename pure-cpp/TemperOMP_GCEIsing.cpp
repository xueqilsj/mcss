...
class TemperOMP_GCEIsing: public Ising {
protected:
	ofstream _fQuan;
...
public:
	...
	virtual ~TemperOMP_GCEIsing() {
		delete[] States_p;
	}
	void _OpenOutPut(std::ios::openmode mode) {
		sprintf(filename,
				"togcei%d_%d~%d~%.15lg_%.15lg_%.15lg_%ld_%ldTraj.txt", nDim0,
				nDim1, nInitconf, dBeta, dAlpha, dUn, _uSamples, _uStride);
		cout << filename << endl;
	...
	///////////
	int Initiate(int dim0, int dim1, int initconf, double dBeta0, double alpha,
			double un0, unsigned long Samples, unsigned long Stride,
			int doflag, int Rank) {
		...
		if (doflag == 0) {
			/*
			sprintf(filename,
					"togcei%d_%d~%d~%.15lg_%.15lg_%.15lg_%ld_%ldLastC.txt",
					dim0, dim1, initconf, dBeta0, alpha, un0, Samples, Stride);
	...
	int RunStatQuan(int dim0, int dim1, int initconf, double dBeta0,
		...
		sprintf(filename, "togcei%d_%d~%d~%.15lg_%.15lg_%02d_%ld_%ldStat.txt",
				nDim0, nDim1, nInitconf, dBeta, dAlpha, nRank, _uSamples,
				_uStride);
	...
	virtual bool RunMetroplisTrialGCE(unsigned long _uOneTimeMoves) {
		...
		while ((_uSampStep + _uOneTimeMoves) >= nN * _uStride) {//sampling
			...
			if (!bContinue) {
				sprintf(filename,
						"togcei%d_%d~%d~%.15lg_%.15lg_%.15lg_%ld_%ldLastC.txt",
						nDim0, nDim1, nInitconf, dBeta, dAlpha, dUn, _uSamples,
						_uStride);
				DumpConf(filename);
			}
	...
	virtual bool TryExchange(TemperOMP_GCEIsing * XNew, double r)//random states
	{
...
void para_temper_SEO(TemperOMP_GCEIsing*mycomms, int nm, int doflag) {
	/*Stochastic even/odd algorithm(SEO),Chemical physics Letters 478(2009)80-84
	 * Ising 16*16,Q=10,\beta_0=1.420,E/N=-17568~-0.8687,N_rep=34, random next neighbor(RNN) 13.69 times of SEO*/
...
#pragma omp parallel shared(nm,setok)
	...
	///task
	if (argc == 12) {
		if (nm != atoi(argv[10])) {///< we need a certain 'np'(number of processes),depending on the size of t[]
			cout << "please set np=" << nm << endl;
			return -1;
		}
		clock_t __start = clock();
		double *un0 = new double[nm];
		double unl = atof(argv[8]);
		double unh = atof(argv[9]);
		double beta0 = atof(argv[6]);
		double alpha = atof(argv[7]);
		unsigned long samples = atoi(argv[4]);
		unsigned long stride = atoi(argv[5]);
		int doflag = atoi(argv[11]);

		for (int i = 0; i < nm - 1; i++)//Linspace
			un0[i] = unl + i * (unh - unl) / (floor(nm) - 1);
		un0[nm - 1] = unh;
		int dim0 = atoi(argv[1]), dim1 = atoi(argv[2]), initconf =
				atoi(argv[3]);

		TemperOMP_GCEIsing *mycomms = new TemperOMP_GCEIsing[nm];

		if (doflag > 0) {
			int k;
#pragma omp parallel for shared(nm)
			for (k = 0; k < nm; k++) {
				mycomms[k].RunStatQuan(dim0, dim1, initconf, beta0, alpha,
			...
			for (int k = 0; k < nm; k++) {
				mycomms[k].Initiate(dim0, dim1, initconf, beta0, alpha, un0[k],
						samples, stride, doflag, k);
			}
		...
		cout
				<< "<TemperOMP_GCEIsing> dim0 dim0  initConf samples stride beta0,alpha,uNlow,uNhigh,nm,doflag"
				<< endl;
	}
	return 1;
}
