...
public:
	...
	virtual bool _NVTDo(unsigned long samp) {
		bool bContinue = true;
		unsigned int QDis[cQ + 1];
		double m1, m2;

		MetroplisTrialRCE(nN * samp, UN0, Arg_io[AI_TEMPER], Alpha);
		...
		if (!bContinue) {
			sprintf(filename,
					"torcep%d_%d_%d~%d~%lf_%.14lf_%lf_%ld_%ldLastC.txt", nDim0,
					nDim1, int(cQ), nInitconf, Arg_io[AI_TEMPER], UN0, Alpha,
					_uSamples, _uStride);
			DumpConf(filename);
		}
...
#pragma omp parallel if(nm > 1) shared(mycomms,nm, un0, beta0, alpha ),private(nRank,continued,lk)
		{
			...
			while (continued) {
				...
				continued = mycomms[nRank]._NVTDo(1);
#pragma omp barrier
				//barrier
#pragma omp single
				{
					for (lk = nRank; lk > 0; lk--) {
						mycomms[lk].TryExchange(&mycomms[lk - 1]);
					}
					for (lk = nRank; lk < nm - 1; lk++) {
						mycomms[lk].TryExchange(&mycomms[lk + 1]);
					}
