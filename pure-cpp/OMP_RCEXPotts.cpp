...
class OMP_RCEPotts: public Potts {
protected:
	ofstream _fQuan;
...
public:
	...
	virtual bool TryExchange(OMP_RCEPotts * XNew, double r)//random states
	{
...
void para_temper_RNN(OMP_RCEPotts*mycomms, int nm, double *un0,
...
#pragma omp parallel if(nm > 1) shared(mycomms,nm, un0, beta0, alpha, gran,uOneTimeMoves,uOldIndexTemper,nTrialMax),private(nRank,continued,lk)
	{
...
void para_temper_SEO(OMP_RCEPotts*mycomms, int nm, double *un0,
		double beta0, double alpha, int doflag) {
...
#pragma omp parallel shared(nm,setok)
	...
	if (argc == 13) {
		...
		OMP_RCEPotts *mycomms = new OMP_RCEPotts[nm];
...
#pragma omp parallel for shared(nm)
		...
		cout
				<< "<OMP_RCEPotts> dim0 dim0 Q initConf samples stride beta0,alpha,uNlow,uNhigh,nm,doflag"
				<< endl;
	}
	return 1;
}
